...
app.get("/",(req,resp)=>{
    if(req.session.username){
        ...
        resp.render(`index.ejs`,{username})
    }
    else{
...
app.get("/products",async(req,resp)=>{
    ...
    resp.render(`products.ejs`,{data,username})
    }
    else if(id==2){
        var db=await connection
        ...
        resp.render(`products.ejs`,{data,username})
            ...
            resp.render(`products.ejs`,{data,username})
                ...
                resp.render(`products.ejs`,{data,username})
                    ...
                    var data=await collection.find({}).toArray()
                    resp.render(`products.ejs`,{data,id,username})
                    }
})
app.get('/product',async(req,resp)=>{
    ...
    if(cid==1){
        ...
        resp.render(`product.ejs`,{data,cid,sm1,sm2,sm3,sm4,collection_name,username})
    }
    else if(cid==2){
...
app.get('/user',async(req,resp)=>{
    ...
    if(req.query.id && username &&req.query.cname){
        ...
        //resp.send(cart_prs)
        //resp.send(prscount)
        resp.render(`user.ejs`,{username,userinfo,cart_prs,prscount})
    ...
    var userinfo=await collection.findOne({'name':username})
    var updateinfo=await collection.updateOne({'name':username},{$set:{'product_id':pid}})
    resp.render(`user.ejs`,{username,userinfo,cart_prs})
    }
}
...
app.post('/Search',async(req,resp)=>{
    ...
    if(result){
        resp.render(`searchresult.ejs`,{result,username})
    }
    else {
...
app.get('/Cover',(req,resp)=>{
    var username=req.session.username
    resp.render(`suitcase_cover.ejs`,{username})
})
app.post('/buynow',async(req,resp)=>{
    ...
    if(collection_id==1){
        ...
        var collection=db.collection('Ladies_Items')
        var data=await collection.findOne({'_id':new ObjectId(id)})
        resp.render(`buynow.ejs`,{data,count,username,userinfo,Message})
    }
